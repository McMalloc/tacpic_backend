...
def append_frontmatter(braille_pages, file_name, order_id)
  braille_pages + "\x0C\x1B\xFC#{linebreak file_name}\x0A\x0ABESTELL-ID: #{order_id}\x1B\xFC"
end
  ...
  def initialize(order)
    ...
    Zip::File.open(@zipfile_name, Zip::File::CREATE) do |zipfile|
      order.order_items.each do |order_item|
        ...
        graphic_file = variant.file_name + '-PRINT-merged.pdf'
        ...
        if order_item.product_id == 'graphic'
          braille_file = variant.file_name + '-BRAILLE.brf'
          braille_contents = File.open(File.join(ENV['APPLICATION_BASE'], 'files', braille_file)).read

          tempfile = Tempfile.new('FM_' + braille_file)
          tempfile.write append_frontmatter(braille_contents, variant.file_name, order.id)
          tempfile.close
